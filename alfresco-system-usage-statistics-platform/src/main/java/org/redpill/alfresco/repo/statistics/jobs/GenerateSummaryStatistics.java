...
public class GenerateSummaryStatistics extends ClusteredExecuter {
  ...
  protected void storeResult(String json) {
    ...
    if (rlNodeRef == null) {
      rlNodeRef = fileFolderService.create(dataDictionaryNodeRef, FOLDER_NAME_REDPILL_LINPRO, ContentModel.TYPE_FOLDER)
          .getNodeRef();
    }
    ...
    NodeRef userStatisticsNodeRef = nodeService.getChildByName(statisticsNodeRef, ContentModel.ASSOC_CONTAINS,
        FOLDER_NAME_SITE_STATISTICS);
    if (userStatisticsNodeRef == null) {
      userStatisticsNodeRef = fileFolderService
          .create(statisticsNodeRef, FOLDER_NAME_SITE_STATISTICS, ContentModel.TYPE_FOLDER).getNodeRef();
    }
    ...
    if (yearNodeRef == null) {
      yearNodeRef = fileFolderService.create(userStatisticsNodeRef, Integer.toString(year), ContentModel.TYPE_FOLDER)
          .getNodeRef();
    }
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    Assert.notNull(summaryReport, "[Assertion failed] - this argument is required; it must not be null");
    Assert.notNull(repository, "[Assertion failed] - this argument is required; it must not be null");
    Assert.notNull(contentService, "[Assertion failed] - this argument is required; it must not be null");
    ...
    Assert.notNull(namespaceService, "[Assertion failed] - this argument is required; it must not be null");
