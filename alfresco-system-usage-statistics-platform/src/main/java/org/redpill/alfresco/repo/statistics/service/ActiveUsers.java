...
public class ActiveUsers implements InitializingBean {
  ...
  public Map<String, List<UserLoginDetails>> getActiveUsersByZone() {
    ...
    for (UserLoginDetails user : activeUsers) {
      ...
      person = personService.getPersonOrNull(user.getUserName());
      if (person == null) {
        LOG.warn("Could not find person " + user.getUserName());
        continue;
      }

      List<ChildAssociationRef> parentAssocs = nodeService.getParentAssocs(person);
      ...
      for (ChildAssociationRef parentAssoc : parentAssocs) {
        if (ContentModel.ASSOC_IN_ZONE.equals(parentAssoc.getTypeQName())) {
          ...
          String property = (String) nodeService.getProperty(parentRef, ContentModel.PROP_NAME);
  ...
  public List<UserLoginDetails> getActiveUsers(long daysBackTrackingLimit) {
    ...
    auditService.auditQuery(callback, parameters, AUDIT_QUERY_MAX_RESULT);
  ...
  public void afterPropertiesSet() throws Exception {
    Assert.notNull(internalUsersZoneName, "[Assertion failed] - this argument is required; it must not be null");
    ...
    Assert.notNull(auditService, "[Assertion failed] - this argument is required; it must not be null");
  }
