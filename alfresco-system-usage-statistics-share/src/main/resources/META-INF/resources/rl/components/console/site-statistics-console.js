  ...
  RL.SiteStatisticsConsole = function(htmlId) {
    ...
    this.LoadAvailableYears = function() {
      ...
      Alfresco.util.Ajax.request({
        url: Alfresco.constants.PROXY_URI + "slingshot/doclib/treenode/node/alfresco/company/home/Redpill-Linpro/Statistics/SiteStatistics?libraryRoot=%2Fapp%3Acompany_home%2Fapp%3Adictionary",
    ...
    this.OnChangeYearFilter = function(evt) {
      ...
      Alfresco.util.Ajax.request({
        url: Alfresco.constants.PROXY_URI + "slingshot/doclib2/doclist/all/node/alfresco/company/home/Redpill-Linpro/Statistics/SiteStatistics/" + target + "?sortField=cm%3acreated&sortAsc=false&libraryRoot=%2Fapp%3Acompany_home%2Fapp%3Adictionary",
        responseContentType: Alfresco.util.Ajax.JSON,
        method: Alfresco.util.Ajax.GET,
        successCallback: {
    ...
    YAHOO.extend(ListPanelHandler, Alfresco.ConsolePanelHandler, {
      ...
      _setupDataStatsTables: function() {
        ...
        var renderCellSize = function(cell, record, column, data) {
          cell.innerHTML = $html(Math.ceil(data / 1024 / 1024));
        };
        ...
        var renderSiteManagers = function(cell, record, column, data) {
          for (var x = 0; x < data.length; x++) {
            var manager = data[x];
            var userid = manager.userName;
            var fullname = manager.fullName.trim();
            var href = Alfresco.constants.URL_PAGECONTEXT + "user/" + userid + "/profile";
            if (cell.innerHTML.length>0) {
              cell.innerHTML += ", ";
            }
            cell.innerHTML += "<a href='" + href + "' target='_blank'>" + fullname + " (" + userid + ")</a>";
          }
        };
        var columnDefinitions = [{
          key: "title",
