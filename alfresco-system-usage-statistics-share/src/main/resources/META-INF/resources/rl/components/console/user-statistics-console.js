...
(function() {
  ...
  RL.UserStatisticsConsole = function(htmlId) {
    ...
    this.fnShowLoadingMessage = function() {
      parent.loadingMessage = Alfresco.util.PopupManager.displayMessage({
        displayTime: 0,
        text: '<span class="wait">' + $html(parent._msg("message.loading")) + '</span>',
        noEscape: true
      });
    };
    ...
    this.LoadAvailableYears = function() {
      ...
      Alfresco.util.Ajax.request({
        url: Alfresco.constants.PROXY_URI + "slingshot/doclib/treenode/node/alfresco/company/home/Redpill-Linpro/Statistics/UserStatistics?libraryRoot=%2Fapp%3Acompany_home%2Fapp%3Adictionary",
    ...
    this.OnChangeYearFilter = function(evt) {
      ...
      Alfresco.util.Ajax.request({
        url: Alfresco.constants.PROXY_URI + "slingshot/doclib2/doclist/all/node/alfresco/company/home/Redpill-Linpro/Statistics/UserStatistics/" + target + "?sortField=cm%3acreated&sortAsc=false&libraryRoot=%2Fapp%3Acompany_home%2Fapp%3Adictionary",
        responseContentType: Alfresco.util.Ajax.JSON,
        method: Alfresco.util.Ajax.GET,
        successCallback: {
  ...
  YAHOO.extend(RL.UserStatisticsConsole, Alfresco.ConsoleTool, {
    ...
    _msg: function(messageId) {
      // 1. Kolla om servern har skickat över översättningen i options
      if (this.options && this.options.messages && this.options.messages[messageId]) {
          return this.options.messages[messageId];
      }
      
      // 2. Original-fallback om något saknas
